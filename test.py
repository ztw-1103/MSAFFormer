...
parser = argparse.ArgumentParser(description='Image Dehazing using MSAFFormer')
...
####### Load yaml #######
yaml_file = 'Options/MSAFFormer.yml'
import yaml
...
print(x)
print(x.get('network_g', 'network_g not found'))

s = x['network_g'].pop('type')
##########################

model_restoration = MSAFFormer(**x['network_g'])
...
factor = 8
datasets = ['Haze1k_moderate']

for dataset in datasets:
    inp_dir = os.path.join(args.input_dir, dataset, 'test', 'input')
    tar_dir = os.path.join(args.input_dir, dataset, 'test', 'target')
...
# datasets = ['Rain100L', 'Rain100H', 'Test1200', 'Test2800']
#
# for dataset in datasets:
#     result_dir  = os.path.join(args.result_dir, dataset)
#     os.makedirs(result_dir, exist_ok=True)
