...
class ImageCleanModel(BaseModel):
    """Generic image restoration model (single-image)."""
    ...
    # ---------- safety utils ----------
    def _assert_weights_finite(self):
        """Raise if any parameter is non-finite."""
        for n, p in self.net_g.named_parameters():
            if p.requires_grad and (not torch.isfinite(p).all()):
                raise RuntimeError(f"Non-finite weights detected in: {n}")

    def _assert_finite(self, tensor, name="tensor"):
        if isinstance(tensor, torch.Tensor) and (not torch.isfinite(tensor).all()):
            raise RuntimeError(f"Non-finite {name} detected.")
    ...
    def init_training_settings(self):
        ...
        if self.ema_decay > 0:
            ...
            if load_path is not None:
                ...
                self.model_ema(0)  # init from net_g
            self.net_g_ema.eval()
    ...
    def optimize_parameters(self, current_iter):
        ...
        # 先检查现有权重是否已经坏掉
        try:
            self._assert_weights_finite()
        except RuntimeError as e:
            print(f"[skip step] {e}  (iter={current_iter})")
            return
        ...
        try:
            for i, p in enumerate(preds):
                self._assert_finite(p, name=f"pred[{i}]")
        except RuntimeError as e:
            print(f"[skip step] {e}  (iter={current_iter})")
            return
        ...
        if not torch.isfinite(l_pix):
            print(f"[skip step] non-finite loss at iter={current_iter}: {l_pix.detach().item()}")
        ...
        if self.opt['train'].get('use_grad_clip', True):
            max_norm = float(self.opt['train'].get('grad_clip_max_norm', 1.0))
            norm_type = float(self.opt['train'].get('grad_clip_norm_type', 2.0))
            torch.nn.utils.clip_grad_norm_(self.net_g.parameters(), max_norm=max_norm, norm_type=norm_type)
    ...
    def pad_test(self, window_size):
        ...
        _, _, h2, w2 = self.output.size()
        self.output = self.output[:, :, 0:h2 - mod_pad_h * scale, 0:w2 - mod_pad_w * scale]
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image):
        # 验证前做一次权重健诊；坏了就跳过本次验证，避免 OOM/崩溃
        try:
            self._assert_weights_finite()
        except RuntimeError as e:
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            # === 关键修改：结果写图前做数值清理，避免 uint8 cast 报 warning/crash ===
            visuals = self.get_current_visuals()
            safe_result = torch.nan_to_num(
                visuals['result'], nan=0.0, posinf=1.0, neginf=-1.0
            ).clamp(0.0, 1.0)
            visuals['result'] = safe_result
